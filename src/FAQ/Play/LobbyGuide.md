...
- 0.7.3.4 版本可以在游戏内点“选项”界面了，并且增加了很多小人形象
- 在电梯和机子前按 `键盘 Z` 交互
- 按住`键盘 A` 加左右方向可以疾跑  
- 按 `Esc` 退出大厅房间
- 进入电梯后可以扩大视野
...
- 按 `F1` 可以快捷发送表情
- 按 `F2` 后，按`数字键 1-9` 可以快捷发送短语，可在“选项”内自定义短语
- 按 `F3` 切换聊天框自动弹出状态，可选三种模式：所有人、仅对战对手、永不自动弹出。默认仅对战对手+公屏气泡，可在“选项”内自定义默认状态
- 消息气泡默认开，可在“选项”内开关
- 打开聊天框后，按 `PageUP` `PageDown` 可翻动聊天记录
- 对战时以及对战结束后，对手的消息高亮显示 5 分钟，默认紫色，可在“选项”内自定义
- 可在此路径找到本地聊天记录 `th123/modules/SokuLobbiesMod/chatlog/`，服务器有全部聊天记录可向管理员询问
- 大厅菜单最后一项的统计，可以查看网络对战中（不管是否通过大厅，只要是网战就都会记录下来）自己的局数、胜率、卡片携带率、卡片使用率等数据
- 增加自动补全功能，使用 /msg /tp 等命令时，使用 tab 键或输入机签可以快速输入
  - /tp 接玩家序号，瞬移至玩家身边（不能在电梯内使用，冷却时间一分钟）
  - /msg 接玩家序号，发送私聊，其他人看不到聊天和气泡
